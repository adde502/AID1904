...
***********************************************
In [154]: regex=re.compile(r"(?P<pig>)cd(ef)")

In [155]: regex.pattern
Out[155]: '(?P<pig>)cd(ef)'

In [156]: regex.groups
Out[156]: 2

In [157]: regex.groupindex
Out[157]: mappingproxy({'pig': 1})
...
pattern=r"(ab)cd(?P<pig>ef)"#捕获组，格式：`(?P<name>pattern)`
regex=re.compile(pattern)
obj=regex.search('abcdefghi')#获取match对象,match='abcdef'
print(obj)
#属性变量
print(obj.pos)#目标字符串的开头位置，返回的下标
print(obj.endpos)#目标字符串的结尾位置，返回的是下标
print(obj.re)#正则
...
print(obj.group(2))#结果：ef,与print(obj.group(pig))结果与子组名一致
***************************************************************************
![结构](img/re1.png)
